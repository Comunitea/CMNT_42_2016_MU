...
class create_mandate(object):
    ...
    def create_mandates(self):
        company_ids = self.search('res.company', [])
        company_data = self.read('res.company', company_ids, ['partner_id'])
        partner_ids = [x['partner_id'][0] for x in company_data]
        bank_ids = self.search('res.partner.bank', [('partner_id', 'not in', partner_ids)])

        for bank_id in bank_ids:
            for company in company_ids:
                mandate_id = self.search('sdd.mandate', [('company_id', '=', company),('partner_bank_id', '=', bank_id)])
                if not mandate_id:
                    self.create('sdd.mandate', {
                        'company_id': company,
